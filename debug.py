...
# -*- coding: utf-8 -*-
"""
Created on Sat Dec 17 20:21:25 2022
...
import cv2 as cv
import sys
sys.path.insert(1, '/home/lino/Documentos/personal/LiDAR_movil') 
...
def explore_match(win, img1, img2, kp_pairs, status = None, H = None):
    ...
    def onmouse(event, x, y, flags, param):
        ...
        if flags & cv.EVENT_FLAG_LBUTTON:
            cur_vis = vis0.copy()
            r = 8
            m = (COMMON.anorm(np.array(p1) - (x, y)) < r) | (COMMON.anorm(np.array(p2) - (x, y)) < r)
            idxs = np.where(m)[0]
...
if __name__ == '__main__':
    ...
    archivo_fotograma_1 = 'fotograma_1.jpeg'
    archivo_fotograma_2 = 'fotograma_2.jpeg'
    # try:
    #     fn1, fn2 = args
    # except:
    #     fn1 = '../data/box.png'
    ...
    img1 = cv.imread(archivo_fotograma_1, 0)
    img2 = cv.imread(archivo_fotograma_2, 0)
    detector, matcher = init_feature(feature_name)

    if img1 is None:
        print('Failed to load fn1:', archivo_fotograma_1)
        sys.exit(1)

    if img2 is None:
        print('Failed to load fn2:', archivo_fotograma_2)
        sys.exit(1)
